...
class Recorder:
    ...
    def __init__(self,
        ...
        # init_value = 1e10
        # # 初始化 self.fc_vals 数组
        # self.fc_vals = np.full(max_feval, init_value)
        # # 原本用inf作为初始化
    ...
    #     try:
    #         # Update function values in fc_vals
    #         self.fc_vals[start_idx : self.last_center + 1] = f_values

    #         # Update minimum found so far and corresponding index
    #         tmp_min_idx = np.argmin(f_values)
    #         if f_values[tmp_min_idx] < self.minimum:
    #             self.minimum = f_values[tmp_min_idx].item()  # Ensure minimum is a scalar
    #             self.best_idx = start_idx + tmp_min_idx
    ...
    #         # Update local slopes
    #         self.local_slope[start_idx:start_idx + nb_fval] = tmp_slope

    #         # Return the largest slope to update the corresponding centres' slope
    #         return tmp_slope.max()
        
    #     except Exception as e:
    #         print(f"Error in update_func_val_and_slope: {e}")
    #         # Optionally, raise the exception to halt execution or handle it appropriately
    #         raise
