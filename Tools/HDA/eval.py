...
from Tools.HDA.yolo_utils import xywh2xyxy, non_max_suppression
...
def _get_batch_statistics(outputs, targets, iou_threshold):
    ...
    for sample_i in range(len(outputs)):
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                iou, box_filtered_index = _bbox_iou(pred_box.unsqueeze(0), torch.stack(filtered_targets)).max(0)
...
def _ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for c in tqdm(unique_classes, desc="Computing AP"):
        ...
        if n_p == 0 and n_gt == 0:
            ...
            ap.append(_compute_ap(recall_curve, precision_curve))
...
def evaluate_yolo(model, dataloader, class_names, img_size, iou_thres=0.5, conf_thres=0.1, 
                  nms_thres=0.5, 
    ...
    for imgs, targets in tqdm(dataloader, desc="Validating"):
        ...
        sample_metrics += _get_batch_statistics(outputs, targets, iou_threshold=iou_thres)
